...
def getProjects():
    try:
        ...
        logging.info(f"GET request to fetch project by user:{user.id} with params:{dict(request.args)} and url:{request.url}")
        if project:
        ...
        logging.info(f"GET request successful, projects returned successfully for user:{user.id}")
...
def getMembers():
    ...
    try:
        project = request.args.get('project')
        user = get_current_user()
        logging.info(f"GET request to fetch all members of the project:{project} by user:{user.id}")
        data = ProjectModel.get_one_project(get_project_id(project,user.user_organization), user.id)
        project_admin_id = data['project_admin']
        data = data['project_members']
        if request.args.get('exclude') == "members":
            all_users = OrganizationModel.get_org_members(organization_id=user.user_organization)
...
def createProjects():
    ...
    try:
        ...
        project_exist = ProjectModel.is_project_exist(data.get('name'),user.user_organization)
        ...
        super_admin = UserModel.query.filter_by(is_super_admin=True,user_organization=user.user_organization).first()
...
def updateName():
    try:
        ...
        project = ProjectModel.query.get(get_project_id(project_name),user.user_organization)
...
def add_members():
    try:
        ...
        logging.info(f"POST request to add members to project by user:{admin.id} with payload:{req_data}")
        project = ProjectModel.is_project_exist(req_data['project'],admin.user_organization)
...
def remove_members():
    try:
        ...
        admin = get_current_user()
        logging.info(f"DELETE request to remove project members by user:{admin.id} with payload:{req_data}")
        project = ProjectModel.is_project_exist(req_data['project'], admin.user_organization)
        if not project:
