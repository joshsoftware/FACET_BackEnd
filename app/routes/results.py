...
def getresults(id=0):
    try:
        ...
        row_size = request.args.get("pageSize") or None
        if has_access_to_project(project, user.id):
            if id!=0:
                data = ResultModel.get_one_result(id)
                return jsonify(data), 200
            if page_no is not None:
                data = ResultModel.get_paginated_results(project_id=project,page_no=page_no,row_size=row_size)
                return jsonify({"results": data}), 200
            else:
                return jsonify({"Error": "Page number or page size not specified"})
        else:
