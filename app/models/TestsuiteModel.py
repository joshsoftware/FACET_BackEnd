...
class TestsuiteModel(db.Model):
    ...
    @staticmethod
    def get_all_testsuites(project):
        data = TestsuiteModel.query.filter_by(project=project)
        data = TestsuiteSchema().dump(data, many=True)
        for testcase in data:
            testcase['created_by'] = get_user_name(id=testcase['created_by'])
            testcase['modified_by'] = get_user_name(id=testcase['modified_by'])
        return data
