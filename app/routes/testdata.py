...
def getTestdata(id=0):
    try:
        ...
        logging.info(
            f"GET request to fetch testdata by user:{get_current_user().id} with params:{dict(request.args)} and url:{request.url}"
        )
        if id != 0:
            data = TestdataModel.get_one_testdata(id)
            logging.info(
                f"GET request successful, testdata returned successfully for testdata id:{id}"
            )
            return jsonify(data), 200

        data = TestdataModel.get_all_testdatas(teststep_id)
        logging.info(
            f"GET request successful, testdatas returned successfully for teststep id:{teststep_id}"
        )
...
def createTestdata():
    try:
        ...
        logging.info(
            f"POST request to create testdata by user:{user.id} with payload:{req_data}"
        )
        ...
        if is_exist:
            ...
            return (
                jsonify(
                    {
                        "error": "You already have a Testdata of the same name in this Teststep."
                    }
                ),
                400,
            )
...
def update_Testdata():
    try:
        ...
        user = get_current_user()
        logging.info(
            f"PUT request to update testdata by user:{user.id} with payload:{req_data}"
        )
        if not testdata:
            logging.info(
                f"PUT request failed as no such testdata exists for the project"
            )
        ...
        if not has_access_to_project(project, user.id):
            ...
            return (
                jsonify(
                    {
                        "error": "You do not have access to this project, kindly connect to project admin to make updates in the project components"
                    }
                ),
                401,
            )
        ...
        testdata.name = (
            create_slug(req_data.get("name")) if req_data.get("name") else testdata.name
        )

        testdata.payload = (
            req_data.get("payload") if req_data.get("payload") else testdata.payload
        )

        testdata.expected_outcome = (
            req_data.get("expected_outcome")
            if req_data.get("expected_outcome")
            else testdata.expected_outcome
        )

        testdata.parameters = (
            req_data.get("parameters")
            if type(req_data.get("parameters")) is dict
            else testdata.parameters
        )
...
def delete_testdata():
    try:
        ...
        user = get_current_user()
        logging.info(
            f"DELETE request to delete testdata by user:{user.id} with payload:{req_data}"
        )
        try:
        ...
        if not testdata:
            logging.info(
                f"DELETE request failed as no such testdata exists for the project"
            )
            return jsonify({"error": "No such Testdata exists"}), 404
        ...
        if not has_access_to_project(project_id, user.id):
            ...
            return (
                jsonify(
                    {
                        "error": "You do not have access to this project, kindly connect to project admin to make deletions in the project components"
                    }
                ),
                401,
            )
